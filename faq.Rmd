...
There is also a `filter_taxa` in `phyloseq`, so the one from the `metacoder` package is masked.
Try prefixing `filter_taxa` with `metacoder::` or loading `metacoder` after `phyloseq`.
...
If you more complex selection criteria and listing them is not practical, you can use a custom function that takes taxon names and returns the inputted name for labels you want printed and `""` or `NA` for labels you want to hide:

```{r eval=FALSE}
name_filter <- function(my_names) {
  ... # Your fancy code, probably using regular expressions
}

heat_tree(
  ...,
  node_label = name_filter(taxon_names),
  ...
)
```

If you specifically want to remove taxa with ambiguous names like "unknown", you can use the `is_ambiguous` function:
...
**I highly recommend against copying large sections of code in this documentation and using it for your analysis without understanding what the code is doing.**
