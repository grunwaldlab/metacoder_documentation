...
```{r silva_load}
library(metacoder)
silva_seq <- read_fasta(file.path(input_folder, "SILVA_123.1_SSURef_Nr99_tax_silva.fasta"))
...
if (! is.null(min_seq_length)) {
  silva <- filter_obs(silva, "tax_data", nchar(sequence) >= min_seq_length, drop_taxa = TRUE)
}
print(silva)
```
...
silva <- mutate_obs(silva, "tax_data", sequence = gsub(pattern = "U", replacement = "T", sequence))
