...
sample_1 <- v35_data %>%
  ...
  heat_tree(
    ...
    node_size = sample_1, 
    node_color = sample_1, 
    node_label = ifelse(sample_1 >= 1, name, NA),
    node_label_max = 3000, 
    node_size_range = c(.01, .07),
    node_label_size_range = c(.015, .1) * 0.4, 
    make_legend = TRUE, 
    node_color_axis_label = "Abundance",
    output_file = file.path(output_folder, paste0("sample_1_tree", output_format)))
sample_2 <- v35_data %>%
  filter_taxa(sample_1 >= min_count | sample_2 >= min_count) %>%
  heat_tree(
    title = "Sample 2",
    ...
    node_size = sample_2, 
    node_color = sample_2, 
    node_label = ifelse(sample_2 >= 1, name, NA),
    node_label_max = 3000, 
    node_size_range = c(.01, .07),
    node_label_size_range = c(.015, .1) * 0.4, 
    make_legend = TRUE, 
    node_color_axis_label = "Abundance",
    output_file = file.path(output_folder, paste0("sample_2_tree", output_format)))
...
bar_plot <- ggplot(bar_data, aes(x = variable, y = value, fill = name)) + 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.text.x=element_text(size=20, margin=margin(b = 5, t = -15)),
        ...
        plot.margin = unit(c(-0.5, 12, 0, -1.5), "lines"))
ggsave(bar_plot, filename = file.path(output_folder, paste0("bar_chart", output_format)), dpi = 350, width = 8, height = 8)
```
...
library(gridExtra)
library(cowplot)
combined_plot <- plot_grid(bar_plot, sample_1, sample_2, ncol = 3, nrow = 1)
save_plot(filename = file.path(output_folder, paste0("barchart_heat_tree_comparison", output_format)),
          combined_plot, ncol = 3, nrow = 1)
```
...
print(matrix_plot)
save_plot(file.path(output_folder, paste0("figure_3--hmp_matrix_plot", output_format)), 
