...
I will calculate read abundance per taxon for later graphing and filtering.
...
calculate_prop_diff <- function(data, site_1, site_2) {
  ...
  result <- ifelse(p_value > 0.05 | is.nan(p_value), 0,
                   log10(vapply(props_1, median, numeric(1)) / vapply(props_2, median, numeric(1))))
  remove_inf(result)
}
...
key_plot <- v35_reduced %>% 
  ...
  heat_tree(node_size = n_obs,
            ...
            overlap_avoidance = 0.7,
            node_size_axis_label = "Number of OTUs",
            node_color_axis_label = "Log 10 ratio of proportions",
            make_legend = TRUE)
```
