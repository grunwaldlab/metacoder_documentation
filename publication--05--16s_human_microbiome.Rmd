...
The last column "Consensus.Lineage" contains the taxonomic classification, so we will use that to parse the rest of the data into a `taxmap` object using the [taxa package](https://github.com/ropensci/taxa).
...
I will save the information on sample characteristics (e.g. body site) in a seperate table, but it could have been included in the `taxmap` object `hmp_data` in the `data` feild.

```{r hmp_sample_data}
sample_data <- utils::read.table(mapping_file, header = TRUE, comment.char = "",
                                 stringsAsFactors = FALSE, sep = "\t")[1:7]
colnames(sample_data) <- c("sample_id", "rsid", "visit_no", "sex", "run_center",
                           "body_site", "description")
sample_data <- dplyr::tbl_df(sample_data)

# Clean up header so that sample IDs match those in the OTU abundance matrix
sample_data$sample_id <- paste0("X", sample_data$sample_id)
...
```{r hmp_treat_comp}
hmp_data$data$diff_table <- compare_treatments(hmp_data,
                                               dataset = "tax_prop",
                                               sample_ids = sample_data$sample_id,
                                               treatments = sample_data$body_site)
```
