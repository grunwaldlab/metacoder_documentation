---
title: "Extracting taxonomy data"
...
* Sequence or abundance data with associated classifications from experiments (e.g. [Genbank](https://www.ncbi.nlm.nih.gov/genbank/) accessions)
* Reference sequence databases (e.g. [Greengenes](http://greengenes.lbl.gov/cgi-bin/nph-index.cgi), [UNITE](https://unite.ut.ee/), [RDP](https://rdp.cme.msu.edu/), [SILVA](https://www.arb-silva.de/))
* Taxonomy databases (e.g. [NCBI taxonomy](https://www.ncbi.nlm.nih.gov/taxonomy), [ITIS](https://www.itis.gov/#))
* Species occuarance databases (e.g. [GBIF](http://www.gbif.org/))

However, these data sources tend to have different ways of specifying taxonomic classifications, including:

* Full classification with taxon names (e.g. Bacteria;Proteobacteria;Gammaproteobacteria; Enterobacterales;Enterobacteriaceae;Escherichia;coli)
* Taxon ID from a reference database (e.g. [NCBI taxon ID 562](https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?mode=Info&id=562&lvl=3&lin=f&keep=1&srchmode=1&unlock))
* Taxon name (e.g. *Escherichia coli*)
* Sequence/Observation ID (e.g. [Genbank accession BA000007](https://www.ncbi.nlm.nih.gov/nuccore/BA000007.2))

In addition, the same method of defining classifications could be encoded with different formats.
For example, all these formats are from refernece databases that supply classifications using a series of taxon names:
...
For all but a full classifcaiton with taxon names (e.g. Bacteria;Proteobacteria;Gammaproteobacteria; Enterobacterales;Enterobacteriaceae;Escherichia;coli), `extract_taxonomy` will query online databases to get the classificaiton. 
Note that this usually invloves quierying NCBI, which is often quite slow, so it it best to use data that has a full classifcaiton with taxon names when possible.
...
> NOTE: `extract_taxonomy` has been designed principally for parsing character vectors, like FASTA headers, rather than tabular data, like the abundance matricies produced by QIIME and mothur. There is another function in development called `parse_taxonomy_table` intended to parse this kind of data, but it is not quite mature yet, so it will not be discussed for now. See `?parse_taxonomy_table` for more information. 
...
Some will be explained in the examples below, but for more details see `?extract_taxonomy`.
...
The `read.FASTA` function from the `ape` [@paradis2004ape] package is commonly used to parse FASTA files in R and `extract_taxonomy` recognizes its output.
...
The format of the UNITE [@koljalg2005unite] FASTA release has two pieces of information from which classifications can be determined.
...
If you want to use the structure and names of the classification provided in the header, but still look up the official taxon id, you can provide `"class"` to `key` and specify a database to look up taxon ids from. 
...
This is prefereable to just using unique taxon names as IDs, since there are often placeholder taxa names such as "unknown" or "spp" and occasionally different taxa can have to same name. 
...
The first observation in the PR2 [@guillou2012protist] header is sometimes a GenBank accession number, but not always.
...
This RDP [@maidak2001rdp] FASTA file does not contain any references to taxon or sequence ids that could be used to look up more information.
...
SILVA [@quast2012silva] has a relativly simple format with an embedded taxonomy:
...
If the input is only the data used to look up classifications, without any other text to seperate from it, the `regex` argument can be omitted.
...
Any list of taxon names can also be parsed by leaving off the regex option.
