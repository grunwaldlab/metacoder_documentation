...
obj %>%
  ...
  filter_taxa(!(taxon_ids %in% subtaxa(obj, taxon_names == "Eukaryota", simplify = TRUE, include_input = TRUE) & n_supertaxa <= 1)) %>%
...
obj <- mutate_obs(obj, "tax_abund", mean_pid = vapply(obs(obj),
                                            function(x) mean(obj$data$tax_data$pid[x], na.rm = TRUE), numeric(1)))
# Percentage of OTUs with less than 90% idententiy
obj <- mutate_obs(obj, "tax_abund", percent_known = vapply(obs(data),
