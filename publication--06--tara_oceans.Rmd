...
metazoa_plot <- data %>%
  ...
  heat_tree(node_color_axis_label = "Percent of OTUs identified",
            ...
            edge_color_range = c("firebrick1", "orangered",  "orange", "yellow3", "yellowgreen", "green"),
            ...
            node_color = percent_known,
            node_color_range = c("firebrick1", "orangered",  "orange", "yellow3", "yellowgreen", "green"),
            node_color_trans = "linear",
            node_color_interval = c(0, 100),
            node_label = ifelse(grepl(pattern = "^[a-zA-z\\-]{1,25}$", name) &
                                  ! Reduce(`|`, lapply(taxa_patterns_to_hide, grepl, x = name)),
                                name, NA),
            node_label_size = (n_obs / (n_supertaxa + 1)) ^ 0.5,
            node_label_size_range = c(0.0085, 0.0125),
            node_label_size_trans = "area",
            node_label_max = 1000,
...
save_plot(filename = file.path(output_folder, paste0("figure_1--tara_metazoa_combined", output_format)),
          combined_plot, ncol = 1, nrow = 2, base_aspect_ratio = 2)
combined_plot
```
