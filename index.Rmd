...
Metacoder is an R package for parsing, plotting, and manipulating hierarchical data.
In particular, metacoder has been designed for large taxonomic data sets, like those generated from modern high-throughput sequencing techniques, like metabarcoding (i.e. amplification metagenomics, 16S metagenomics, etc).
...
* Sources of taxonomic data can be extracted from any file format and manipulated using [dpylr](https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html)-like functions that take into account hierarchical relationships. 
...
The function that runs *in silico* PCR requires `primersearch` from the EMBOSS [@rice2000emboss] tool kit to be installed. 
...
Taxonomic data can be extracted from most file formats using the **extract_taxonomy** function.
Classifications can be parsed offline or retrieved from online databases if a taxon name, taxon ID, or sequence ID is used.
A regular expression with capture groups and a corresponding key is used to define how to parse the file.
The example code below parses the 16s Ribosome Database Project training set for Mothur which can be found here:
...
The `taxon_funcs` part contains functions (e.g. `n_obs`) used to calculate taxon statistics.
These can be used like a column in `data$taxon_data` in most of metacoder's functions, expect they are calculated on demand.
Note how `n_obs` is used in the following examples.
...
Metacoder maps taxonomic data (e.g. sequence abundance) to color/size of tree components in what we call a **heat tree**:
...
The default size range displayed is optimized for each plot to minimize overlaps, but can also be customized by the user.
...
```{r home_plot_3}
set.seed(1)
filter_taxa(data, name == "Archaea", subtaxa = TRUE) %>%
...
This makes it easy to remove lower ranks (e.g. species) of a taxonomy without discarding observations assigned to the tips: 
...
Random samples from large reference databases are biased towards over-represented taxa.
...
The code below estimates the coverage of the universal prokaryotic primer pair 515F/806R [@walters2016improved]: 
...
* the coverage of different primer pairs
...
* The `taxmap` object and associated filtering functions like `filter_taxa` are being separated into the rOpenSci package [taxa](https://github.com/ropensci/taxa), which we hope to make a taxonomy standard for R that other packages can use. 
...
Metacoder's major dependencies are taxize, igraph, dplyr, and ggplot2.
